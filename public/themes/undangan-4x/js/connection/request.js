...
export const pool = (() => {
    ...
    const ensurePool = (lists) => {
        ...
        for (const name of lists) {
            if (!cachePool.has(name)) {
                cachePool.set(name, createNullCache());
            }
        }
    };
    ...
    return {
         ...
         */
        restart: async (name) => {
            if (!window.isSecureContext || !hasCacheApi()) {
                ensurePool([name]);
                return;
            }
         ...
         */
        init: (callback, lists = []) => {
            const cacheNames = lists.concat([cacheRequest]);

            if (!window.isSecureContext || !hasCacheApi()) {
                ensurePool(cacheNames);
                callback();
                return;
            }

            cachePool = new Map();
            Promise.all(cacheNames.map((v) => window.caches.open(v).then((c) => cachePool.set(v, c)))).
                then(() => callback()).
                catch(() => {
                    ensurePool(cacheNames);
                    callback();
