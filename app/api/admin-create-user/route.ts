...
export async function POST(req: Request) {
  ...
  if (!secret || !safeEq(provided, secret)) return new Response('Unauthorized', { status: 401 });
  ...
  try {
    ...
    const { error: upErr } = await admin
      .from('profiles')
      .upsert({ user_id: user.id, email, is_admin, role: is_admin ? 'admin' : 'client' }, { onConflict: 'user_id' });
